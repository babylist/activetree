...
RSpec.describe ActiveTree::RootQuery do
  subject(:query) { described_class.new(model_name, query_str) }
  ...
  describe "#result" do
    context "with blank model" do
      ...
      it "raises ArgumentError" do
        expect { query.result }.to raise_error(ArgumentError, "Model class is required")
      end
    end
    ...
    context "with blank query" do
      ...
      it "raises ArgumentError" do
        expect { query.result }.to raise_error(ArgumentError, "Query is required")
      end
    end
    ...
    context "with unknown model" do
      ...
      it "raises ArgumentError" do
        expect { query.result }.to raise_error(ArgumentError, "Model 'NoSuchModel' not found")
      end
    end
    ...
    context "with numeric query" do
      ...
      it "raises when record not found" do
        ...
        expect { query.result }.to raise_error(ArgumentError, /not found/)
      end
    end

    context "with DSL query returning one record" do
      let(:query_str) { "where(active: true)" }
      let(:record) { double("Record", id: 1) }
      let(:dsl_relation) { double("DslRelation") }

      before do
        allow(mock_relation).to receive(:instance_eval).with(query_str).and_return(dsl_relation)
    ...
    context "with DSL query returning multiple records" do
      ...
      before do
        call_count = 0
        allow(mock_relation).to receive(:instance_eval).with(query_str) do
          call_count += 1
    ...
    context "with DSL query returning no results" do
      ...
      it "raises ArgumentError" do
        expect { query.result }.to raise_error(ArgumentError, /No results/)
      end
    end
